...
struct ProjectDetailView: View {
    ...
    private var filteredTasks: [Item] {
        let pool = project.taskArray.filter { !archivingIDs.contains($0.objectID) }
        switch selectedStatus {
        ...
        ZStack(alignment: .topLeading) {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(showsIndicators: false) {
                    LazyVStack(spacing: 5) {
                        if filteredTasks.isEmpty {
                            Text(project.taskArray.isEmpty
                ...
                TBAddFAB(label: "Add task to \(project.name ?? "project")", action: onAddTask)
            }
    ...
    private var miniFolderBanner: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                ZStack {
                    RoundedRectangle(cornerRadius: 7)
                        .fill(project.color.opacity(0.2)).frame(width: 32, height: 32)
                    Image(systemName: "folder.fill")
                        .font(.system(size: 15)).foregroundColor(project.color)
                }
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    HStack(spacing: 4) {
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                RoundedRectangle(cornerRadius: 99)
                                    .fill(Color.Surface.a30.opacity(0.4)).frame(height: 3)
                                RoundedRectangle(cornerRadius: 99)
                                    .fill(project.color)
                                    .frame(width: geo.size.width * CGFloat(project.progress), height: 3)
                            }
                        ...
                        .frame(height: 3)
                        Text("\(Int(project.progress * 100))%")
                            .font(.custom("Montserrat-Bold", size: 9)).foregroundColor(project.color)
                    }
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                ...
                VStack(alignment: .trailing, spacing: 3) {
                    Text("\(project.completedCount) done")
                        .font(.custom("Montserrat-Regular", size: 9)).foregroundColor(Color.Surface.a50)
                    Text("\(project.taskCount - project.completedCount) left")
                        .font(.custom("Montserrat-Bold", size: 10)).foregroundColor(AppColors.textWhite)
                }

                Button(action: onEditProject) {
                    Image(systemName: "square.and.pencil")
                        .font(.system(size: 11)).foregroundColor(Color.Surface.a50)
                }
                .buttonStyle(.plain)
            }
            .padding(.horizontal, 12).padding(.vertical, 10)
            .background(project.color.opacity(0.1))
            .overlay(RoundedRectangle(cornerRadius: 0).stroke(Color.white.opacity(0.07), lineWidth: 0.5))
            .cornerRadius(9, corners: [.bottomLeft, .bottomRight, .topRight])
        }
            ...
            ForEach(ProjectTaskStatus.allCases) { s in
                Button { selectedStatus = s } label: {
                    Text(s.rawValue)
                        ...
                        .overlay(Capsule().stroke(selectedStatus == s ? AppColors.shalyPurple : Color.Surface.a30.opacity(0.5), lineWidth: 0.5))
