...
struct ProjectInfoView: View {
    ...
    @State private var selectedColorHex = "#BC6CD9"
    @State private var priority: TaskPriority = .medium
    @State private var deadline: Date? = nil
    ...
    private var wordCountColor: Color {
        wordCount >= 280 ? Color.Danger.a10 : wordCount >= 230 ? Color.Warning.a10 : Color.Surface.a50
    }
    ...
    var body: some View {
        ...
        .background(AppColors.background)
        .frame(width: 330, height: expandNotes ? 520 : 440)
        .onAppear { populateFromProject() }
        .sheet(isPresented: $showDeadlinePicker) {
            VStack(spacing: 16) {
                Text("Deadline").font(.custom("Montserrat-Bold", size: 15)).foregroundColor(AppColors.textWhite)
                DatePicker("", selection: $deadlineDraft, displayedComponents: .date)
                ...
                HStack {
                    ...
                    Button("Done") { deadline = deadlineDraft; showDeadlinePicker = false }
                        .buttonStyle(.glassProminent)
                }
            }
    ...
    private var headerBar: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text(isEditing ? "Edit Project" : "New Project")
                    .font(.custom("Montserrat-Bold", size: 16)).foregroundColor(AppColors.textWhite)
                Spacer()
                Circle()
                    .fill(Color(hex: selectedColorHex))
                    .frame(width: 18, height: 18)
    ...
    private var colorSection: some View {
        sectionWrap {
            ...
            HStack(spacing: 8) {
                ForEach(ProjectPalette.swatches, id: \.hex) { swatch in
                    Button { selectedColorHex = swatch.hex } label: {
                        ZStack {
    ...
    private var notesSection: some View {
        sectionWrap {
            HStack {
                ...
                HStack(spacing: 3) {
                    Text("\(wordCount)")
                        .font(.custom("Montserrat-Bold", size: 9)).foregroundColor(wordCountColor)
                    Text("/ \(maxNotesWords)")
                        .font(.custom("Montserrat-Regular", size: 9)).foregroundColor(Color.Surface.a50)
                }
                // Expand toggle
                Button {
                    withAnimation(.easeInOut(duration: 0.2)) { expandNotes.toggle() }
                } label: {
                    Image(systemName: expandNotes ? "arrow.down.right.and.arrow.up.left" : "arrow.up.left.and.arrow.down.right")
                        .font(.system(size: 10)).foregroundColor(Color.Surface.a50)
                }
                .buttonStyle(.plain)
                .padding(.leading, 6)
            }
            RichTextEditor(text: notesBinding)
                .frame(minHeight: expandNotes ? 120 : 55, maxHeight: expandNotes ? 200 : 80)
                .padding(6)
                .background(Color.Surface.a10)
                .cornerRadius(8)
    ...
    private var notesBinding: Binding<String> {
        Binding(get: { notes }, set: { notes = clampedNotes($0) })
    }
                ...
                ForEach(TaskPriority.allCases) { p in
                    Button { priority = p } label: {
                        Text(p.shortLabel)
                            ...
                            .background(Capsule().fill(priority == p ? p.color.opacity(0.22) : Color.Surface.a20.opacity(0.3)))
                            .overlay(Capsule().stroke(priority == p ? p.color : Color.Surface.a30.opacity(0.5), lineWidth: 0.5))
                            .foregroundColor(priority == p ? p.color : Color.Surface.a60)
                    }
                    .buttonStyle(.plain)
                }
        ...
        sectionWrap {
            ...
            Button {
                ...
                HStack {
                    if let d = deadline {
                        Text(d, style: .date)
                            .font(.custom("Montserrat-Medium", size: 11)).foregroundColor(AppColors.textWhite)
                    } else {
                        Text("Set deadline")
                            .font(.custom("Montserrat-Regular", size: 11)).foregroundColor(Color.Surface.a50)
                    }
                    Spacer()
                    Image(systemName: "calendar").font(.system(size: 11)).foregroundColor(Color.Surface.a50)
                }
                ...
                HStack(spacing: 8) {
                    Group {
                        if let app = selectedApp {
                            ...
                            LinkIconView(link: linkURL).frame(width: 18, height: 18)
                        } else {
                            ZStack {
                                RoundedRectangle(cornerRadius: 4)
                                    .fill(Color.Surface.a20.opacity(0.5)).frame(width: 18, height: 18)
                                Image(systemName: "link.badge.plus")
                    ...
                    if let app = selectedApp {
                        ...
                        Text(linkURL)
                            .font(.custom("Montserrat-Regular", size: 10)).foregroundColor(AppColors.textWhite).lineLimit(1)
                    } else {
                        Text("Attach app or link")
                            .font(.custom("Montserrat-Regular", size: 11)).foregroundColor(Color.Surface.a50)
                    }
                    Spacer()
                    if selectedApp != nil || !linkURL.isEmpty {
                        Button { selectedApp = nil; linkURL = "" } label: {
                            Image(systemName: "xmark.circle.fill")
                                .font(.system(size: 12)).foregroundColor(Color.Surface.a40)
                        }
                        .buttonStyle(.plain)
                    }
                }
    ...
    private func sectionWrap<C: View>(@ViewBuilder content: () -> C) -> some View {
    ...
    private var divider: some View {
        Divider().background(Color.Surface.a30.opacity(0.35)).padding(.horizontal, 14)
    }
    ...
    private func saveProject() {
        guard !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return }
        let proj = project ?? Project(context: viewContext)
        if project == nil { proj.id = UUID(); proj.timestamp = Date() }
        proj.name = name.trimmingCharacters(in: .whitespacesAndNewlines)
        proj.notes = notes.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? nil : notes
        proj.colorHex = selectedColorHex
        proj.priority = priority.rawValue
