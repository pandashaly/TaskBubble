...
enum ProjectTaskStatus: String, CaseIterable, Identifiable {
    case all = "All", todo = "To Do", doing = "Doing", done = "Done"
    var id: String { rawValue }
}
...
struct ProjectDashboardView: View {
    ...
    @Environment(\.managedObjectContext) private var viewContext
    @FetchRequest(sortDescriptors: [NSSortDescriptor(keyPath: \Project.timestamp, ascending: true)], animation: .default)
    private var projects: FetchedResults<Project>
    ...
    private var allProjectItems: [Item] {
        items.filter { ($0.value(forKey: "project") as? Project) != nil }
            .sorted { ($0.timestamp ?? .distantPast) > ($1.timestamp ?? .distantPast) } // newest first
    }
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            AppColors.background.ignoresSafeArea()
            VStack(spacing: 0) {
                TBPageHeader(title: "Projects", icon: "folder.fill",
                             onBack: onBack,
                             onNavDrawer: { withAnimation { showNavDrawer.toggle() } },
                             onSearch: onSearch, onAdd: onAddTask,
                             accentColor: Color.Purple.normal)
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                        projectsSection
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text("PROJECTS")
                    ...
                    .tracking(1)
                Spacer()
                // This is the "Add Project" button in the header
                Button(action: onAddProject) {
                    HStack(spacing: 4) {
                        Image(systemName: "plus").font(.system(size: 9, weight: .bold))
                        Text("Add project").font(.custom("Montserrat-Bold", size: 9))
                    }
                    .foregroundColor(AppColors.shalyPurple)
                }
                .buttonStyle(.plain)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 9) {
                    // 1. Your existing projects
                    ForEach(projects) { project in
                        ProjectFolderCard(project: project)
                            .onTapGesture { onSelectProject(project) }
                    }
                    
                    // 2. The NEW "Add Project" placeholder card
                    AddProjectCard(action: onAddProject)
                }
    ...
    private var taskListSection: some View {
        LazyVStack(spacing: 5) {
            if filteredItems.isEmpty {
                Text(allProjectItems.isEmpty ? "Add tasks inside a project to see them here" : "No tasks in this filter")
                    .font(.custom("Montserrat-Regular", size: 12)).foregroundColor(Color.Surface.a50)
                    .frame(maxWidth: .infinity).padding(.top, 20).multilineTextAlignment(.center).padding(.horizontal, 30)
            } else {
                ForEach(filteredItems) { item in
                    let proj = item.value(forKey: "project") as? Project
                    ProjectTaskRow(
...
// MARK: - Project task row (with subtask warning on completion)
...
struct ProjectTaskRow: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            Rectangle().fill(projectColor).frame(width: 3)
                .cornerRadius(9, corners: [.topLeft, .bottomLeft])
            ...
            HStack(spacing: 7) {
                Button(action: {
                    ...
                    ZStack {
                        Circle().stroke(item.completed ? Color.Success.a0 : Color.Surface.a30, lineWidth: 1.5).frame(width: 15, height: 15)
                        if item.completed {
                            Circle().fill(Color.Success.a0.opacity(0.15)).frame(width: 15, height: 15)
                            Image(systemName: "checkmark").font(.system(size: 7, weight: .bold)).foregroundColor(Color.Success.a0)
                        }
                    }
                }
                .buttonStyle(.plain)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    HStack(spacing: 5) {
                        ...
                        if isArchiving {
                            Text("archiving…").font(.custom("Montserrat-Regular", size: 9)).foregroundColor(Color.Success.a0.opacity(0.7))
                        }
                        SubtaskIndicator(item: item)
                    }
                }
                ...
                Button(action: onSelect) {
                    Image(systemName: "info.circle").font(.system(size: 11)).foregroundColor(Color.Surface.a40)
                }
                .buttonStyle(.plain)
            }
            .padding(.vertical, 7).padding(.trailing, 9).padding(.leading, 7)
...
struct ProjectStatusBadge: View {
    ...
    var body: some View {
        let (label, bg, fg) = info
        Text(label).font(.custom("Montserrat-Bold", size: 8))
            .padding(.horizontal, 6).padding(.vertical, 2)
...
struct AppLinkChip: View {
    let resourceType: String; let resourceValue: String
    @ObservedObject var appDetectionService: AppDetectionService
    var body: some View {
        Group {
            if resourceType == LinkedResourceType.app.rawValue,
               let icon = appDetectionService.getIcon(for: resourceValue) {
                Image(nsImage: icon).resizable().scaledToFit().frame(width: 14, height: 14).cornerRadius(3)
            } else if resourceType == LinkedResourceType.url.rawValue {
                LinkIconView(link: resourceValue).frame(width: 14, height: 14)
            } else {
                Image(systemName: "app").font(.system(size: 9)).foregroundColor(Color.Surface.a50)
        ...
        .frame(width: 20, height: 20).background(Color.Surface.a20.opacity(0.5)).cornerRadius(4).padding(.trailing, 3)
    }
}

// MARK: - Corner helpers (keep in one place)
...
struct RectCorner: OptionSet {
    let rawValue: Int
    static let topLeft = RectCorner(rawValue: 1 << 0), topRight = RectCorner(rawValue: 1 << 1)
    static let bottomLeft = RectCorner(rawValue: 1 << 2), bottomRight = RectCorner(rawValue: 1 << 3)
    static let all: RectCorner = [.topLeft, .topRight, .bottomLeft, .bottomRight]
}
